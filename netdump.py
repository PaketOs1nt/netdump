...
globalthreads = min(_cpu_count * 64, 1024)
localthreads = min(_cpu_count * 2, 32)
lock = threading.Lock()
...
class Network:
    ...
    def getservice(port: int) -> str:
        try:portname = socket.getservbyport(port, 'tcp')
        except:portname = 'unknown'
...
def main():
    ...
    for addr in ip_ranges:
        if addr.endswith('/8'):
            globalthreads = min(_cpu_count * 64, 512)
